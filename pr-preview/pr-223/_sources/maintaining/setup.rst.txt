...
`Contributing Guidelines <https://github.com/hasansezertasan/peta/blob/main/.github/CONTRIBUTING.md>`_.
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/peta \
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/peta/branches/main/protection \
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/peta/actions/permissions/workflow \
...
.. code-block:: sh
   ...
   gh api --silent \
     repos/hasansezertasan/peta/dependency-graph/compare/main...main
...
.. code-block:: sh
   ...
   gh api -X PUT repos/hasansezertasan/peta/environments/publish \
     -F 'deployment_branch_policy[protected_branches]=false' \
     -F 'deployment_branch_policy[custom_branch_policies]=true'
   ids="$(gh api --paginate repos/hasansezertasan/peta/environments/publish/deployment-branch-policies \
     --jq '.branch_policies[].id')"
   for id in $ids; do
     gh api -X DELETE "repos/hasansezertasan/peta/environments/publish/deployment-branch-policies/$id"
   done
   gh api -X POST repos/hasansezertasan/peta/environments/publish/deployment-branch-policies \
...
.. code-block:: sh
   ...
   gh variable set ENABLE_PRIVATE_ATTESTATIONS --body true --repo hasansezertasan/peta
...
.. code-block:: sh
   ...
   policies="$(gh api --paginate repos/hasansezertasan/peta/environments/publish/deployment-branch-policies \
...
.. code-block:: sh
   ...
   priv="$(gh repo view hasansezertasan/peta --json isPrivate --jq '.isPrivate' 2>/dev/null)"
   if [ "$priv" = "false" ]; then true
   elif [ "$priv" = "true" ]; then
     gh secret list --repo hasansezertasan/peta --json name --jq '.[].name' | grep -qx CODECOV_TOKEN
   else false; fi
...
.. code-block:: sh
   ...
   gh secret set GITLEAKS_LICENSE --repo hasansezertasan/peta
...
.. code-block:: sh
   ...
   owner="$(gh api repos/hasansezertasan/peta --jq '.owner.type' 2>/dev/null)"
   if [ "$owner" = "User" ]; then true
   elif [ "$owner" = "Organization" ]; then
     gh secret list --repo hasansezertasan/peta --json name --jq '.[].name' | grep -qx GITLEAKS_LICENSE
   else false; fi
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/peta/pages \
...
.. code-block:: sh
   ...
   gh api -X PATCH repos/hasansezertasan/peta -F has_discussions=true

**[CHECK]**

.. code-block:: sh

   [ "$(gh repo view hasansezertasan/peta --json hasDiscussionsEnabled --jq '.hasDiscussionsEnabled')" = "true" ]
...
``https://repology.org/project/peta/versions`` and verify that
...
``peta``. If it does, use the name shown by Repology in both
URLs below.
...
.. code-block:: markdown
   ...
   [![Packaging status](https://repology.org/badge/vertical-allrepos/peta.svg)](https://repology.org/project/peta/versions)
