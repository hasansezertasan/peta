...
def test_compare_marks_vulnerabilities_unknown_for_any_advisory_source() -> None:
    failed = _pkg(
        name="httpx",
        version="0.27.0",
        enrichment_failures=[
    ...
    output = format_compare(_pkg(), failed)
    # An alternate advisory provider failing must not render as a clean "0".
    assert "Vulnerabilities\t0\tunknown" in output
