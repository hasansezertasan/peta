...
| Task | Command |
| ---- | ------- |
| Run one test file | `uv run --locked pytest tests/smoke/test_smoke.py -v` |
...
**Do not create new top-level subpackages under `src/peta/`.**
...
- Source lives in `src/peta/`; tests are grouped by kind under
  `tests/{unit,integration,e2e,smoke}/`, matching the pytest markers.
- `src/peta/_version.py` is generated by hatch-vcs — never edit it.
